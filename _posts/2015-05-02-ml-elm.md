...
where $$C$$ is regularization term, $$\sigma\_{1}>0$$ and $$\sigma_{2}>0$$, $$p$$ and $$q$$ is indicating the norm. The above function is clearly a Linear Regression formulation. Note that unlike conventional neural networks, there is no activation function (or say it's a linear activation) to output layer. It's not hard to say that we can use Gradient Descent to find the representation of $$W_{o}$$. However, ELM offered an analytical solution when $$p=q=\sigma_{1}=\sigma_{2}=2$$:
